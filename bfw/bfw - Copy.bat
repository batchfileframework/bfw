...
Call :GetLabelRow "%_GFR_BatchFile%" %_GFR_FunctionName% _GFR_FunctionStart
Call :GetFunctionExit "%_GFR_BatchFile%" %_GFR_FunctionStart% _GFR_FunctionExit
Call :GetFunctionPreambleRow "%_GFR_BatchFile%" %_GFR_FunctionStart% _GFR_FunctionPreamble
Call :GetFunctionPostscriptRow "%_GFR_BatchFile%" %_GFR_FunctionStart% _GFR_FunctionPostscript
Set "%_GFR_OutputObject%.name=%_GFR_FunctionName%"
Set "%_GFR_OutputObject%.preamble=%_GFR_FunctionPreamble%"
Set "%_GFR_OutputObject%.start=%_GFR_FunctionStart%"
Set "%_GFR_OutputObject%.exit=%_GFR_FunctionExit%"
Set "%_GFR_OutputObject%.postscript=%_GFR_FunctionPostscript%"
...
:GetLabelRow
for /f delims^=:^ tokens^=1 %%a in ('%SystemRoot%\System32\findstr /N "^:%~2" "%~1"') do ( if "[%~3]" NEQ "[]" set "%~3=%%a" & exit /b %%a )
exit /b 0
...
set "_GFE_Function=%~2"
set "_GFE_Output=%~3"
echo.%_GFE_Function%| findstr /r "[^0123456789]" >nul && Call :GetFunctionName %_GFE_BatchFile% _GFE_Function _GFE_FunctionName || set "_GFE_FunctionName=%_GFE_Function%" & Call :GetLabelRow %_GFE_BatchFile% %_GFE_Function% _GFE_Function
...
:GetFunctionName
for /F "usebackq eol= tokens=1,* delims=:" %%i in (`^(type %~1 ^| findstr /n /r /c:".*" ^| findstr /B /C:"%~2:" ^) 2^>nul`) do ( set "%~3=%%j" & exit /b 0 )
exit /b 1
...
for /f "delims=- tokens=1,2" %%a in ("%~2") do ( set "_AFLTF_Start=%%a" & set "_AFLTF_Stop=%%b"  )
...
for /f %_AFLTF_skip% delims^=^ eol^= %%a in (' ^( type "%_AFLTF_File%" ^| %SystemRoot%\System32\findstr /N /R /C:".*" ^) 2^>nul ') do ( 
